...
module.exports = async (req, res) => {
    try {
        ...
        if(ENDPOINT === '') return res.status(400).json({msg:'Debe configurar una ciudad de búsqueda.'});

        const clasification = await Category.find({tkm_id: segmentId})

        if (!clasification) return res.status(400).json({msg:'Debe ingresar una catégoria de búsqueda.'});

        ENDPOINT = ENDPOINT + `&classificationId=${segmentId}`;
        
        ENDPOINT = startDate !== '' && endDate !== '' ? ENDPOINT + `&localStartEndDateTime=${startDate},${endDate}` : '';

        if(ENDPOINT === '') return res.status(400).json({msg:'Debe configurar un rango de fechas.'});
